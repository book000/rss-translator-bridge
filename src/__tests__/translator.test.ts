...
describe('Translator - Batch Translation', () => {
  ...
  it('should return original texts on non-200 status', async () => {
    const mockResponse = {
      status: 500,
      data: {},
    }
  ...
  it('should return original texts when response status is false', async () => {
    const mockResponse = {
      status: 200,
      data: {
        status: false,
    ...
    expect(result.size).toBe(0) // When status is false, no fallback is applied unless there's an error
  })
  ...
  it('should handle axios error with response data', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error response data:', { error: 'Rate limit exceeded' })
    expect(consoleSpy).toHaveBeenCalledWith('Error response status:', 429)
